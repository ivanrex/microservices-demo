...
class RecommendationService(demo_pb2_grpc.RecommendationServiceServicer):
    def ListRecommendations(self, request, context):
        ...
        try:
            ...
            logger.info(
                "event=recommendations_returned service=recommendationservice component=grpc "
                "action=list_recommendations entity=recommendation reason=list_recommendations outcome=success "
                f"recommendation_count={len(prod_list)}"
            )
            return response
            ...
            logger.warn(
                "event=recommendations_failed service=recommendationservice component=grpc "
                "action=list_recommendations entity=recommendation reason=list_recommendations outcome=failure "
                f"error={exc}"
            )
            raise
